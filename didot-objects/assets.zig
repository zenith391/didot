...
pub const AssetStream = struct {
    ...
    pub fn fileStreamPath(path: []const u8) !AssetStream {
        return fileStream(try std.fs.cwd().openFile(path, .{ .read = true }));
    }

    pub fn fileStream(streamFile: std.fs.File) !AssetStream {
        ...
        const dupe = try std.heap.page_allocator.create(File);
        dupe.* = streamFile;
...
pub const Asset = struct {
    /// Pointer to object
    objectPtr: usize = 0,
    /// Allocator for the asset itself and the object
    ...
    pub fn preload(self: *Asset, allocator: *Allocator) !void {
        if (std.io.is_async) {
            ...
            std.log.scoped(.asset).debug("preload() called althought I/O is blocking.", .{});
        }
    }
...
pub const AssetManager = struct {
    ...
    fn assetFromExtension(allocator: *Allocator, ext: []const u8) !?Asset {
        inline for (textureExtensions) |expected| {
            if (std.mem.eql(u8, ext, expected)) {
                return try TextureAsset.init2D(allocator, expected[1..]);
            }
        }
        inline for (meshExtensions) |expected| {
            if (std.mem.eql(u8, ext, expected)) {
                return try MeshAsset.init(allocator, expected[1..]);
            }
        }
        return null;
    }

    pub fn loadFromBundle(self: *AssetManager, bundle: std.fs.File, allocator: *Allocator) !void {
        const compression = try bundle.reader().readByte(u8);

        comptime var compressionType = 0;
        inline while (compressionType < 1) : (compressionType += 1) {
            if (compression == compressionType) {
                var stream = switch (compressionType) {
                    0 => bundle, // no compression
                    1 => try std.compress.zlib.zlibStream(allocator, bundle.reader()) // zlib (deflate) compression
                };
                const reader = stream.reader();

                const entries = try reader.readIntLittle(u64);
                var i: u64 = 0;
                while (i < entries) : (i += 1) {
                    const pathLength = try reader.readIntLittle(u32);
                    const dataLength = try reader.readIntLittle(u32);

                    var path = try allocator.alloc(u8, pathLength);
                    _ = try reader.readAll(path); // read the path

                    var data = try allocator.alloc(u8, dataLength);
                    _ = try reader.readAll(data);

                    var asset: ?Asset = try assetFromExtension(allocator, std.fs.path.extension(path));
                    if (asset) |ast| {
                        ast.stream = try AssetStream.bufferStream(data);
                        try self.put(path, ast);
                    } else {
                        std.log.warn("No corresponding asset type for bundled {s}", .{ path });
                    }
                }
            }
        }
    ...
    pub fn autoLoad(self: *AssetManager, allocator: *Allocator) !void {
        ...
        while (try walker.next()) |entry| {
            if (entry.kind == .File) {
                var asset: ?Asset = try assetFromExtension(allocator, std.fs.path.extension(entry.basename));
                if (asset) |*ast| {
                    std.log.info("{s}", .{ entry.path });
                    const file = try dir.openFile(entry.path, .{});
                    ast.stream = try AssetStream.fileStream(file);
                    try self.put(try allocator.dupe(u8, entry.path), ast.*);
                } else {
                    std.log.warn("No corresponding asset type for {s}", .{ entry.path });
                }
            }
        }
    }
    ...
    pub fn deinit(self: *AssetManager) void {
        var iterator = self.assets.valueIterator();
